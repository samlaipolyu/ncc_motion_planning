...
x_tip = 41*sign((cos(tspan))).*(sin(tspan)).*cos(tspan).^2;
y_tip = 41*sign(cos(tspan)).*cos(tspan).^2;
...
for i = 1:length(viaPoint) 
    ...
    A = [1 1 1 1 1 1 (10^-6)];                            % can be customized
    b = 10^-4;                                            % can be customized
    ub = [-eps; -eps; -eps; -eps; -eps; -eps; 60];        % can be customized
    lb = -[2; 2; 2; 2; 2; 2; 2];                          % can be customized
    
    option = optimoptions('fmincon','Display','final','Algorithm','interior-point'); % or `sqp`
...
for i = 1:length(x)
    ...
    [pcplot, tip(:,i), sim_angle(:,i)] = drawAnimation(x(:,i),[0 0 0]);
    hold on;
...
while j <= 8*length(x)/8
    tic
    for i = 1:8*length(x)/8
        ...
        F = f_11 + f_12 + f_13 + f_21 + f_22 + f_23;
        ...
        K_A1 = 0.8*pi*(r_o^2-r_i^2)/(50*(1+0.45*((f_11 + f_12 + f_13 + f_21 + f_22 + f_23))/(0.8*pi*(r_o^2-r_i^2))));
        K_A2 = 0.8*pi*(r_o^2-r_i^2)/(50*(1+0.45*((f_21 + f_22 + f_23))/(0.8*pi*(r_o^2-r_i^2))));
        r = 3.15;
        ...
        theta_2 = theta_2(end);
        phi_1 = atan2(3*(f_12-f_13),sqrt(3)*(f_12+f_13-2*f_11));
        ...
        subplot(4,2,[1 3 5 7])
        plot3(x_01,y_01,z_01,'-','LineWidth',4,'color','[0, 0.4470, 0.7410]');hold on
        ...
        sim_angle(:,j) = transpose(rotm2eul(R_02{50}));
        ...
        plot3(x_02,y_02,z_02,'-','LineWidth',4,'color','[0.9290, 0.6940, 0.1250]');hold on
...
%% Check the deviation in 3-axis orientation
figure(7)
subplot(3,1,1)
hold on
